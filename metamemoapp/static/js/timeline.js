$(document).ready(function () {
    ...
    $.each( timelines, function( key, val ) {
      ...
      timeline_selectors.push(`
                <div class="ovo col offset-s3 s6 m3 padding-top50 center-align">
                    <a data-name="${val["name"]}" id="${nome_id}" href="#${nome_id}" class="white-text">
      ...
      $.each(facts, function(this_tl_key,this_tl_facts){
        ...
        $(`#timeline-${decodeURI(this_tl_facts["name"].split(' ').join('-'))}`).append(`
        ...
        let preDefinedPerson = window.location.href.split("#")[1];
        if (preDefinedPerson !== undefined) {
          let personAnchor = $(`#${decodeURI(preDefinedPerson)}`)[0];
          if (personAnchor !== undefined) {
            //let linkToSearch = $("#link-to-search");
            //let authorName = $(personAnchor).data("name");
            //$('.timeline-body').removeClass('active');
            //$(`#timeline-${this.id}`).addClass('active');
            //linkToSearch.attr("href", linkToSearch.data().href + `?author=${authorName}`);
            $(personAnchor).click();
          }
        }
      ...
      $('.timeline-seletor a').each(function(index, el) {
        $(this).click(function(event) {
          ...
          $('.timeline-body').removeClass('active');
          $(`#timeline-${decodeURI(this.id)}`).addClass('active');
          linkToSearch.attr("href", linkToSearch.data().href + `?author=${authorName}`);
        });
      });
  ...
  $('.timeline-body').removeClass('active');
  $(`#timeline-${decodeURI(window.location.hash.substring(1))}`).addClass('active');
});
