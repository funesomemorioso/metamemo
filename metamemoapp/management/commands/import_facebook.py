...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not cookies_file or not os.path.isfile(cookies_file):
            ...
            raise
        input_posts = facebook_scraper.get_posts(
            username, pages=pages, options={"posts_per_page": ppp, "allow_extra_requests": False}, cookies=cookies_file
        )
        ...
        for i in input_posts:
            if i["post_id"] in memo_itens:
                ...
                if i["video"]:
                    post.medias.create(
                        original_url=f"http://facebook.com/{i['video_id']}", original_id=i["video_id"], status="INITIAL"
                    )
